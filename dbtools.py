...
class Dao(object):
    ...
    def update(self, set, cond):
        ...
        cond_names = cond.keys()
        cond_values = cond.values()
        params = list(set_values)+list(cond_values)
        stmt = 'UPDATE {} SET {} WHERE {} '.format(self._table_name,','.join([set + '=?' for set in set_names]), ' AND '.join([col + '=?' for col in cond_names]))
        self._conn.cursor().execute(stmt, params)
